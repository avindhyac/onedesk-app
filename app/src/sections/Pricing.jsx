...
const PLANS = [
  {
    ...
    features: ["Company formation", "Registered agent", "Monthly bookkeeping", "Annual report filing", "Email support"],
  ...
  },
  {
    ...
    featured: true,
    features: ["Everything in Starter", "Dedicated accountant", "Tax filing & planning", "Legal contract review", "Priority support"],
    cta: "Choose Growth",
  },
  {
    name: "Full Desk",
    ...
    features: ["Everything in Growth", "HR & payroll", "Marketing pod", "Quarterly business review", "Dedicated success lead"],
    cta: "Go Full Desk",
  },
];
              ...
              <thead>
                <tr>
                  ...
                  {["Starter", "Growth", "Full Desk"].map((n, i) => (
                    <th key={n} className={`pricing-table__th ${i === 1 ? "pricing-table__th--featured" : ""}`}>{n}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {COMPARE_ROWS.map((r) => (
                  <tr key={r[0]} className="pricing-table__row">
                    <td className="pricing-table__td pricing-table__td--label">{r[0]}</td>
                    <td className="pricing-table__td">{cell(r[1])}</td>
                    <td className="pricing-table__td pricing-table__td--featured">{cell(r[2])}</td>
        ...
        <div className="pricing-faq__inner">
          ...
          <div className="pricing-faq__list">
            {PRICING_FAQS.map((f, i) => (
              <FaqItem key={f.q} q={f.q} defaultOpen={i === 0}>{f.a}</FaqItem>
            ))}
          </div>
          <p className="pricing-faq__note">
            All plans billed monthly. <Link to="/contact" className="pricing-faq__link">Talk to us</Link> if you need something custom.
          </p>
        </div>
      </section>
    </>
