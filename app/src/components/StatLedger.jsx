...
function reserve(item) {
  const str = item.to !== undefined
    ? item.to.toLocaleString() + (item.suffix || "")
    : String(item.value ?? "");
  return Math.max(str.length, 2);
}

export default function StatLedger({ items, variant = "light", layout = "row", className = "" }) {
  const accents = variant === "dark" ? ACCENT_DARK : ACCENT_LIGHT;

  return (
    <div className={`od-ledger od-ledger--${variant} od-ledger--${layout} ${className}`.trim()}>
      {items.map((s, i) => (
        <RevealOnScroll key={s.label} className="od-ledger__cell" delay={i * 0.09} y={16}>
          <span
            className="od-ledger__tick"
            style={{ background: accents[s.accent] || accents.ember }}
            aria-hidden="true"
          />
          <div className="od-ledger__value" style={{ minWidth: `${reserve(s)}ch` }}>
            {s.to !== undefined
